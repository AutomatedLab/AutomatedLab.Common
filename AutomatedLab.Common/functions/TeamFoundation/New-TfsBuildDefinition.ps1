...
{
    [CmdletBinding(DefaultParameterSetName = 'Cred', SupportsShouldProcess = $true, ConfirmImpact = 'Medium')]
    param
    (
    ...
    if (-not $PSCmdlet.ShouldProcess($ProjectName, "New build definition")) { return }
    ...
    try
    {
        $null = Get-TfsProject @projectParameters
    }
    catch
    {
        Write-Error -ErrorRecord $_
